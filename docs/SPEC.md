...
| review | 리뷰·회신 요구 | 지정 대상 | 7일 |
| memo | 개인 기록 | 본인만 | 없음 |
| risk | 기술 위험 플래그 | 팀 전체 | 즉시 상단 고정 |

임계값은 `config` 테이블로 분리 (코드 배포 없이 조정).

**생명주기**: `제기됨 → 논의중 → 결정됨 → 반영됨(Task 생성)` 또는 `기각됨`. 반드시 종료된다.
...
2. **팀 타임라인 (시그니처)** — 캘린더 2층 레인, 현재 시각 인디케이터
3. 이번 달 목표 진척
4. 마감 임박 테이블
5. 시그널 패널 (타입 필터)
6. 허들 피드
...
| 테이블 | 주요 컬럼 |
|---|---|
| `actor` | id PK, type(human\|agent), display_name, owner_actor_id FK, avatar_url, is_active |
| `account` | actor_id PK, email, password_hash, role(lead\|member\|viewer), must_change_pw, last_login_at |
...
| `task` | id PK, project_id FK, title, description, status, assignee_id FK, due_date, priority, origin(human\|agent), created_by FK, completed_at |
...
| `signal` | id PK, type, scope, title, body, author_id FK, project_id FK, task_id FK, status, resolved_at |
...
| `signal.status` | open / discussing / resolved / archived |
...
| 역할 | 범위 |
|---|---|
| lead | 계정 발급·비활성화, 프로젝트·목표 생성·삭제, 결정 확정, 월간 보고 승인, Notion 연동 범위 변경 |
| member | Task·시그널 전권, 본인 부사수 초안 승인, 허들 참여 |
| viewer | 읽기 + 코멘트 |

- 계정은 lead가 직접 발급 (이메일 + 임시 비밀번호 → 최초 로그인 시 변경 강제)
- 퇴사는 삭제가 아닌 `is_active = false` (담당자 이력 보존)
...
## 11. 운영 전환 체크리스트 (기존 유지)

- [ ] Notion 통합 토큰 발급 (워크스페이스 소유자 권한 필요)
- [ ] OpenAI 법인 결제 → API 키 발급
- [ ] 시드 이메일을 실제 업무메일로 교체
- [ ] 팀원 초기 비밀번호 변경 안내
- [ ] Notion 타임라인 DB 속성 허용값 확인 (구분·업무유형·상태·우선순위)
