...
**제품이 원본, Notion은 백업 (D-011 강화).** 업무 기록·확인의 주 무대는 팀보드이며 Notion은 승인 확정본의 백업 미러다.

- Notion 미연결(`NOTION_TOKEN` 없음) 상태에서도 모든 기능이 완전히 동작해야 한다.
- Notion 오류가 업무 등록·수정·상태 변경을 막아서는 안 된다 (승인의 Notion 기록만 예외적으로 실패 처리).
- 변경 작업 후에는 `NOTION_TOKEN` 없는 상태로 전 화면 1회 확인을 원칙으로 한다.
...
| **D-023** | **업무 상세는 우측 슬라이드 패널로 통일 (모달 금지), 어느 화면에서든 동일 진입** | 모달은 목록 맥락을 가림. `?task=id` URL 반영 + 필드 이탈 시 인라인 자동저장으로 편집 마찰 제거 |
| **D-024** | **연구소 내부용 3차 확대 + 배경<카드<입력요소 3단 명도 위계 (순수 검정 금지, 최저 #0A0E17)** | 100%가 적정 밀도가 되도록 토큰만 확대. 다크에서 카드 층 분리를 명도 위계 + top 하이라이트로 확보 |
| **D-025** | **홈 밀도 재배분 — 1순위(타임라인·지연마감·시그널) 크게 / 2순위(프로젝트·목표) 링 게이지 요약 후 클릭 확대 / 3순위(허들) 하단 접힘** | 한 화면에 6요소를 같은 크기로 두면 우선순위가 사라짐. 요약+확대로 정보 밀도와 스캔성 양립 |
| **D-026** | **빠른 입력 3경로 — "+ 새로 만들기" 드롭다운 / ⌘K 제목만 즉시 생성 / 캘린더 빈 칸 추가** | 생성 진입이 한 곳뿐이면 마찰. ⌘K 빠른 생성은 영역을 서버 기본값(본인 소속 영역)으로 배치해 제목만으로 완결 |
| **D-027** | **영역 작업 공간(/areas/[key]) + 영역별 기본 업무유형은 config 저장 (코드 하드코딩 금지)** | 영역마다 운영 방식이 달라 기본 업무유형을 코드에 고정하면 변경마다 재배포. is_active=false 영역은 404 |
| **D-028** | **업무 코멘트(task_comment)를 시그널 코멘트와 별도 테이블로 분리** | 업무 논의와 시그널 논의는 수명·가시성 규칙이 달라 한 테이블에 섞으면 조회·권한이 꼬임 |
| **D-029** | **데모 데이터는 is_demo 순수 부가 플래그로 격리, "데모 비우기"는 기존 소프트 삭제(is_active=false)로 처리** | 어떤 조회 쿼리도 is_demo를 필터하지 않아 실운영 안정화 후 컬럼 DROP만으로 제거 가능. 운영 시드(actor/account/area/config/project)는 대상 제외 |
| **D-030** | **모든 주요 목록·패널의 빈 상태는 안내 문구 + 첫 행동 유도(EmptyState), 일러스트는 파일 있을 때만** | 실데이터 초기의 빈 화면이 고장처럼 보이면 입력을 시작하지 않음. 일러스트 파일 부재 시 onError로 슬롯만 접고 문구는 항상 노출 |
| **D-031** | **승인 시 Notion 기록 실패가 승인 자체를 막지 않음 (D-011 미러 원칙) — 저장 확정 + 오류 표면화 + activity_log 기록** | 우리 DB가 원본. Notion 장애로 업무 흐름이 멈추면 안 됨. 실패는 notion_page_id NULL + error 로그 + UI 경고로 드러내고 재시도 유도 |
| **D-032** | **스키마 반영은 db/migrations 번호순 자동 마이그레이션 — 배포 후 최초 DB 접근이 미적용분만 적용(schema_migrations 이력, advisory lock)** | 콘솔·임시 라우트(init-db) 수동 호출 제거. 배포만으로 반영. 비파괴(ADD/CREATE)만 권장, ALLOW_DB_INIT/x-admin-secret 폐기 → lead 세션 인증으로 교체 |
| **D-033** | **인수인계(handover)는 담당자별 문서 + 업무 연결, status='shared'로만 타인 공개(lead·해당 영역 담당)** | 퇴사·휴가·이관 대비. 편집은 작성자만, 삭제는 소프트. 마크다운 에디터·업무 다중 선택·PDF(@media print)는 기존 컴포넌트 재사용 |
| **D-034** | **Notion은 선택 연동 — NOTION_TOKEN 없으면 관련 UI(승인 모달 문구·타임라인·설정)를 숨기고 승인은 자체 DB에만 확정** | 토큰 없이도 "안 되는 기능"으로 보이지 않게. 토큰이 생기면 UI·미러링이 자동 복귀. 자체 DB 기록·업무 흐름은 어느 경우든 동일 |
| **D-035** | **영역 재편 — workspace 5 + 교육자료(link_only, Notion 링크만) + 기타(비활성). area.kind/notion_url 도입** | 이전 배치에서 누락된 재편을 마이그레이션(0003)으로 확정. link_only는 업무·목표 선택지·작업공간에서 제외, 사이드바에서 "(링크)"로 표기 |
| **D-036** | **목표에 scope(team/personal) 도입 — 팀 목표(lead 편집·전원 공유)와 개인 목표(본인만, lead도 조회 불가)** | 개인 목표를 팀에 노출하면 심리적 부담. goal_task 연결은 재사용해 한 업무가 팀+개인 목표에 동시 연결 가능 |
| **D-037** | **분기·연간 진척은 하위 가중평균(연결 업무 수 가중), 진척은 goal.progress에 이벤트 기반 저장** | 단순평균은 업무가 몰린 하위를 과소반영. 업무 done/dropped·연결 변경 시 recomputeGoalChain으로 체인만 재계산(조회 시 전체 재계산 금지) → 홈·목표에 즉시 반영 |
| **D-038** | **허들룸 회의 모드 = 회의록 — 몰입형 전체화면(좌 코멘트 실시간 / 우 결정사항), 종료 시 decision 승격** | 별도 회의록 도구 없이 논의 그 자체가 기록으로 남게. 그리기·실시간 커서는 범위 밖(펜 없음) |
| **D-039** | **허들룸 투표(huddle_vote 👍👎)·==하이라이트==·외부 이미지 URL 첨부** | S3 업로드 없이 외부 URL만(범위 밖 회피). 하이라이트는 텍스트 마크업(그리기 아님) |
...
2. **Notion 타임라인 DB 속성 허용값 확인** — ✅ **스키마 대조 완료 (2026-07)**
   - 실제 스키마 반영 완료: "구분" 속성 없음(전송 제거), "업무 구분"은 단일 select, "상태"는 status 타입.
   - 코드는 `lib/notion-schema.ts` 폴백 + Notion 동적 캐시 사용. 토큰 발급 후 `GET /api/admin/verify-notion-schema`로 최종 대조 + 담당자 person ID 매핑 확인.
