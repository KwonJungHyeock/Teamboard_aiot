...
### 「왜 갑자기 관리자 권한이 빠졌지」 — 시드가 되돌리고 있었다 (MD-P-2026-063 §B-1)

로컬 시드(`scripts/init-db.mjs`)가 **돌 때마다 역할과 권한을 제 값으로 덮어쓴다.**
063 이전에는 그 표가 실제 결정과 어긋나 있었다:

- 시드의 권정혁이 `role: "lead"` 였다. DB 는 `admin` 인데 시드를 다시 돌리면
  **관리자가 팀장으로 강등됐다.**
- 시드가 `admin_grant` 를 아예 안 썼다. 0035 부트스트랩이 한 번 켠 권한이
  시드 재실행 뒤에도 남아 있는지 **아무도 보증하지 못했다.**

063 에서 시드가 `role` 과 `adminGrant` 를 **모든 줄에 명시**하게 고쳤다.
그러니 **권한이 갑자기 빠졌다면 시드가 돌았는지부터 본다.** 그리고 이 표는
서 있는 결정(지금은 「권정혁·박정길 둘만 모두관리자」)과 같아야 한다 —
결정이 바뀌면 시드도 같이 바꾼다. `admin-grant-walk` 의 `0-모두관리자는-둘` 이
그것을 센다.

같은 자리의 더 큰 함정 하나 — `init-db` 는 `db/schema.sql` 을 **다시 적용**하고,
그 파일에는 DDL 만 있는 게 아니라 `UPDATE area …` 같은 **데이터 문장**이 있다.
시드를 돌리는 것은 스키마 작업이 아니라 **데이터 작업**이다(§G 063).
