...
export default function SignalThread({
  ...
  // 코멘트 이미지 첨부 — 공용 첨부 한 벌 (MD-P-2026-026 §B-2).
  const up = useAttach({ kind: "signal", id: signalId }, (uploaded, file) => {
    setAttach(uploaded);
  ...
  return (
    <section className={`card sthread ${signal.agent ? "agline" : ""}`} aria-label="논의 스레드">
      ...
      {/* 대화형 답글 스레드 */}
      <DropZone className="thread" onFile={up.send} disabled={editing !== null}>
        ...
        <div className="cmt-tools">
          <AttachButton onFile={up.send} busy={up.busy} disabled={editing !== null}
            reason={editing !== null ? "편집 중에는 첨부할 수 없어요" : undefined} />
        </div>
        <AttachStatus state={up.state} onRetry={up.retry} onDismiss={up.dismiss} />
        {/* 상시 하단 컴포저 */}
