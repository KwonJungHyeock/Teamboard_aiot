-- 로컬 DB 기준. 프로덕션 데이터와 불일치 확인됨(2026-08-06). 재작성 전 실행 금지.
-- 사유: 이 파일은 목표 #9~#15 · 프로젝트 1·2·3 을 전제로 하지만 프로덕션 목표 트리는 다르다.
--       tb_sqltest 통과는 로컬 복제본 기준이라 프로덕션 안전을 보장하지 않는다.
--       01_verify_before.sql 결과를 받은 뒤 프로덕션 실데이터 기준으로 재작성한다.
-- ============================================================
