...
const CASES = [
   ...
   */
  /*
   * 062 §F — **이 단언은 지금 죽어 있다.** 027 §B 가 콤보박스를 버튼 줄로
   * 바꾸면서 「새 프로젝트로 만들기」 줄이 없어졌고, 팀장에게도 안 보인다.
   * 지우지 않고 둔다 — 검사기가 제 입으로 죽었다고 말하는 편이 조용한 통과보다
   * 낫다. 되살릴 시점과 조건은 `docs/BACKLOG.md` 의
   * 「모달에서 프로젝트 새로 만들기」 줄에 `task-create-walk` 의 지운 검사 둘과
   * **한 표에** 적어 뒀다 — 셋이 같은 기능 하나를 재던 것이라 따로 두면
   * 되살릴 때 하나를 빠뜨린다.
   */
