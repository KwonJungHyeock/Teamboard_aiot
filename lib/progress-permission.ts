...
//
//   ① **권한** — 지정된 한 사람이 아니다
//   ② **규칙** — 하위 업무가 있으면 진척은 하위 **완료 개수**다 (lib/progress.ts 규칙 2)
//
...
export interface ProgressEditSubject {
  /**
   * **집계 대상** 하위 업무 수 — 취소·중복 하위는 빠진다.
   *
   * ── 왜 전체 수가 아니라 이것인가 (MD-P-2026-036 §B) ──────────────
   *
   * 계산은 `child_counted` 를 보는데 판정은 `child_count`(전체)를 보고 있었다.
   * 두 값이 다른데 같은 것으로 취급한 탓에, **하위가 전부 취소·중복인 업무**가
   * 이런 상태가 됐다.
   *
   *   effectiveProgress = 자기 값 (하위로 계산되지 **않는다**)
   *   그런데 PATCH → 400 「하위 업무가 있어 하위 완료 개수로 계산됩니다」
   *
   * **막는 이유가 사실이 아니었고, 그 업무의 진척은 아무도 못 바꿨다.**
   * 규칙은 「하위로 **계산될 때** 손으로 못 바꾼다」이므로, 판정도 계산이 보는
   * 값을 봐야 한다. 규칙을 바꾼 것이 아니라 규칙을 지키게 한 것이다.
   */
...
export function canEditProgress(
  ...
  if ((task.childCount ?? 0) > 0) {
    return {
      ...
      // 「완료율」이 아니라 **「완료 개수」**다 — 하위의 진척 값은 안 쓴다.
      // 화면 문구(「하위 N개 중 M개 완료」)와 같은 말을 한다.
      message: "하위 업무가 있어 진척은 하위 완료 개수로 계산됩니다. 손으로 바꿀 수 없습니다.",
    };
  }
