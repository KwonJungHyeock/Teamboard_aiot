...
/**
 * **기한 막대의 기본 눈금 범위** (§C 중간 병합 · §1).
 *
 * 화면이 각자 범위를 정하면 갈린다 — 실제로 갈렸다. 홈은 기간 토글에 연결해
 * stub 0% 였는데 `/tasks` 는 이번 달 고정이라 **50%** 였다. 같은 병이 두 화면에서
 * 다르게 났고, §C3·§D 에서 화면이 늘면 또 갈릴 자리였다.
 *
 * > **눈금 범위는 컴포넌트의 기본값이다. 화면이 덮어쓰려면 이유를 적는다.**
 *
 * 지금 덮어쓰는 곳은 하나 — 홈이다. 홈에는 「이번 분기 / 전체 기간」 토글이 있고,
 * 그 토글이 곧 사용자가 고른 범위라서 컴포넌트 기본값보다 세다.
 */
export function defaultBarRange(today: string): BarRange {
