...
-- ══════════════════════════════════════════════════════════════════
--
-- 배치 ③은 시간이 지날수록 되돌리기 어려워진다. 「상시 프로젝트에 붙은 업무」가
-- 마이그레이션이 붙인 것과 **사람이 화면에서 붙인 것**으로 섞이고, SQL 은 둘을
-- 구별하지 못한다.
--
-- > **하루 안에 이상이 없으면 되돌리기 옵션은 닫힌 것으로 본다.**
-- > 그 뒤에는 앞으로만 간다 — 되돌리는 대신 새 번호로 정방향 마이그레이션을 쓴다.
--
-- 시한이 지난 뒤 정말 되돌려야 한다면, 아래 ③을 그대로 돌리지 말고
-- **무엇이 마이그레이션 것이고 무엇이 사람 것인지 먼저 가른다.**
-- 배포 직후 사후 확인에서 뜬 「만들어진 상시 프로젝트 목록」이 그때 유일한 근거다.
--
