...
실제 Notion DB 스키마(2026-07 확인)와 일치. 코드 폴백 단일 소스는 `lib/notion-schema.ts`이며,
실제 선택지는 Notion에서 동적 조회해 캐시합니다(팀장이 Notion 속성값을 바꿔도 재배포 불필요).

| 속성 | 타입 | 허용값 |
|---|---|---|
| 업무명 | title | (초안 제목 — `[업무 구분]` 접두어 자동 삽입, 중복 방지) |
| 업무 구분 | **미확정(select/multi_select)** | 기타 · R&D · 플랫폼 · 연구소 · 디자인 · 교육자료 · 현장실습교육 |
| 상태 | **status** | 대기 · 진행 · 완료 (status 타입 — API로 옵션 추가/삭제 불가, 이름만 지정) |
| 우선순위 | select | High · Mid · Low |
| 업무유형 | select | 팀업무 · 개인업무 · 상시업무 |
| 담당자 | people | (account.notion_user_id 매핑) |
| 시작일 / 종료일 | date | (업무 기간 — 시작일·종료일 별개 속성) |
| 메모 | rich_text | (초안 한 줄 요약 — 목록 뷰용) |

> "구분"(팀 메인/개인 상시) 속성은 실제 DB에 없어 전송하지 않습니다.
> **"업무 구분"의 select/multi_select 여부는 스크린샷으로 확정 불가**하므로, 페이로드는 Notion에서 조회한 **실제 타입으로 분기**합니다(`buildPropertyValue`). 타입 확정 전(토큰·캐시 없음)에는 승인이 "스키마 새로고침을 실행하세요" 안내로 안전하게 막힙니다. 데이터 모델은 `workAreas: string[]` 배열로 유지되어 어느 타입이든 코드 변경이 없습니다. `/settings`의 "Notion 스키마 새로고침"으로 캐시를 갱신하며, 타입·값 변경 시 drift 경고가 표시됩니다.
